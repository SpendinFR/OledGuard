...
internal sealed partial class MonitorSession : IDisposable
{
        ...
        string foregroundTitle)
    {
        ...
        lock (_sync)
        {
            ...
            if (titleChanged)
            {
                // Un changement d'onglet ou une navigation peut conserver
                // le même HWND. On le traite comme une nouvelle scène afin
                // que la fenêtre active soit lisible pendant trois secondes.
                ResetSceneToBaseline(
                    current,
                    now,
                    foregroundWindow,
